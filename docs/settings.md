...
`cyberModels` declares the models the operator approves for cyber work. `cyberMode` enables this allowlist for the first session of a process. Every role resolves inside the list, and the session refuses model switches outside it. The `cyber` status-line segment shows the state. This feature does not bypass provider safety restrictions. See [Cyber mode](./cyber-mode.md) for filtering and lifecycle rules.
...
| `plan.keybindingWorkflow` | enum | `parallel` | `parallel`, `debate`. Selects which workflow the plan shortcut enters from the off state. |
