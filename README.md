...
Nine roles route work by intent. `default` for normal turns. `smol` for cheap subagent fan-out. `slow` for deep reasoning. `plan` for plan mode. `commit` for changelogs. Plus `vision`, `task`, `advisor`, and `tiny` for their namesakes. Override at launch with `--smol`, `--slow`, or `--plan`; cycle through the configured models for the active role with `Ctrl+P`. Swap the active model mid-session with the `/model` slash command. Bundle whole role sets under a name with [`modelProfiles`](docs/models.md#model-profiles), start on one via `modelProfile:` or `--model-profile`, and swap the set with `Alt+Shift+M`.
