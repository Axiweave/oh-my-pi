...
export function resolutionDeviceUsage(device: ResolutionDeviceName): string {
	switch (device) {
		...
		case PROPOSE_DEVICE_NAME:
			return `Write your plan's <slug> (matching local://<slug>-plan.md) as plain text to ${PROPOSE_DEVICE_PATH} to submit the plan for approval (plan mode) or the finished implementation for independent review (after a debate plan is approved).`;
	}
}
