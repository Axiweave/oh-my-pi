...
export class MCPManager {
	...
	async #doReconnect(
		...
		if (oldConnection) {
			...
			// The IDE bridge is excluded: its lockfile poll already owns
			// discovery and pacing (#scheduleIdeReconnectPoll), so a second
			// schedule would double every reconnect attempt.
			if (
				source?.provider !== "ide" &&
				(config.type === "http" || config.type === "sse") &&
				!this.#lostRemoteServers.has(name)
			) {
