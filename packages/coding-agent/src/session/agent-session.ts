...
import { cfgPlanDebateMaxRounds } from "../plan-mode/settings";
import {
...
import { cfgDisabledProviders, cfgModelProviderOrder, cfgModelRoles } from "../config/model-settings";
...
export class AgentSession implements SettingsScope {
	...
	constructor(config: AgentSessionConfig) {
		...
		this.#planDebateGate = new PlanDebateGate({
			...
			maxRounds: cfgPlanDebateMaxRounds.get(this.settings),
		});
		...
		// lands unchanged reports nothing. Report before the settings edit returns.
		this.addDisposer(
			this.settings.onEffectiveChange([cfgModelRoles, cfgModelProviderOrder], () => {
	...
	buildDisplaySessionContext(): SessionContext {
		return this.#withEvalStateContext(
			this.#preserveActivePlanModeContext(this.#providerBoundary.buildDisplaySessionContext()),
		);
	}
	...
	async #dispatchPrompt(
		...
		if (this.isStreaming) {
			...
			await this.#queueUserMessage(
				...
				options?.images,
				streamingBehavior,
				{ timestamp: submittedAt, attribution: promptAttribution },
				commandCardName,
				commandCardInput,
			);
	...
	async navigateTree(
		...
		const displayContext = this.#withEvalStateContext(
			this.#preserveActivePlanModeContext(deobfuscateSessionContext(stateContext, this.#obfuscator)),
		);
