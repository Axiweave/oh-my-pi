...
SELECT h.id, h.prompt, l.created_at, l.cwd, l.session_id, h.use_count
...
ON CONFLICT(prompt, cwd) DO UPDATE SET
	 ...
	 * with the use counts of every collapsed row summed, and recreates both
	 * tables from {@link HISTORY_TABLE_DDL}. Subsumes every legacy shape at
	 * once — unixepoch defaults, missing session_id, non-unique prompt,
	...
	#rebuildHistory(): boolean {
		...
		this.#db.transaction(() => {
			...
			for (const row of winners.values()) {
				insertHistory.run(row.id, row.prompt, row.created_at, row.cwd, row.session_id, row.use_count);
			}
