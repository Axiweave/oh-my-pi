...
import {
	...
	cfgDisplayCollapseCommandCards,
...
const cfgLiveUiSettings = combine({
	...
	"tui.pinComposerBottom": cfgTuiPinComposerBottom,
	"display.streamingScrollback": cfgDisplayStreamingScrollback,
	...
	"display.collapseCommandCards": cfgDisplayCollapseCommandCards,
...
export class InteractiveMode implements InteractiveModeContext {
	...
	#liveComposerPreferences(): Omit<ComposerPreferences, "quiet" | "composerShape"> {
		return {
			...
			pinBottom: cfgTuiPinComposerBottom.get(this.settings),
			streamingScrollback: cfgDisplayStreamingScrollback.get(this.settings),
		};
	}
	...
	syncComposerShape(): void {
		...
		setComposerStylePreferences({ footerMode: cfgComposerStyleFooterMode.get(settings) });
	...
	async #reconcileModeFromSession(options?: { preserveActiveGoal?: boolean }): Promise<void> {
		...
		if (sessionContext.mode === "plan") {
			...
			if (!cfgPlanImplReview.get(this.session.settings)) {
	...
	showPlanReview(
		...
		if (cfgAskNotify.get(this.session.settings) !== "off") {
	...
	async #approvePlan(
		...
		const implReview = options.debate && cfgPlanImplReview.get(this.session.settings);
	...
	async handleWorktreeMoveCommand(targetPath?: string): Promise<void> {
		if (this.#vibeSessionTransitionBlocked()) return;
		await this.#commandController.handleWorktreeMoveCommand(targetPath);
	}
