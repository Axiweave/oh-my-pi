...
import { cfgCycleOrder, cfgModelProfileSwitchStyle } from "../../config/model-settings";
import {
...
import { cfgPlanKeybindingWorkflow } from "../../plan-mode/settings";
...
export class InputController {
		...
		for (const key of planModeKeys) {
			this.ctx.editor.setCustomKeyHandler(key, () => {
				const requestedWorkflow = this.ctx.planModePaused
					? "parallel"
					: (this.ctx.session.getPlanModeState()?.workflow ?? cfgPlanKeybindingWorkflow.get(this.ctx.settings));
	...
	async cycleModelProfile(direction: "forward" | "backward" = "forward"): Promise<void> {
		if (cfgModelProfileSwitchStyle.get(this.ctx.settings) === "picker") {
