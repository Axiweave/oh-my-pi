...
export class Settings {
	...
	setProjectCyberMode(enabled: boolean): void {
		const prev = cfgCyberMode.get(this);
	...
	getModelProfiles(): ModelProfilesSettings {
		const profiles: unknown = cfgModelProfiles.get(this);
	...
	installStartupModelProfile(options?: { force?: boolean }): string | undefined {
		const name = cfgModelProfile.get(this)?.trim();
	...
	#rebuildMerged(): void {
		...
		const previousRawModelRoles = this.#preCyberModelRoles;
		...
		// Report raw role edits even when allowlist filtering keeps the effective
		// model unchanged.
		if (this.#cyber && !settingValuesEqual(previousRawModelRoles, this.#preCyberModelRoles)) {
			this.#notifyChange(cfgModelRoles);
		}
	}
