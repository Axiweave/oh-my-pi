...
- OMP now sends a `session_state_changed` notification (`idle`, `working`, `needs-input`, `done`, `failed`) to the connected IDE MCP server, so editors such as claude-code-ide.el can show whether a session waits for input or finished a turn. Only the main session reports: viewing a subagent, returning from it, or closing a dialog while idle no longer overwrites a finished turn with `idle`. A `/guided-goal` interview question reports `needs-input` instead of `done` until `goal create` runs. The payload carries the session working directory, and a move (`/wt`, `/move`, persistent `!cd`, cross-project `/resume`) republishes the unchanged state so the editor can relabel the session with its new path and branch.
- OMP now accepts `ESC _ pi:prompt;<command> ESC \` input packets to replace or insert the active composer's leading slash command. In an empty composer the caret lands after the inserted command and its trailing space, ready for arguments.
- `ESC _ pi:prompt;<command> ESC \` packets naming a wrapper command (`queue`, `plan`, `debate`, `vibe`) nest it in front of the draft's leading command instead of replacing it, so a `/skill:x …` draft becomes `/queue /skill:x …`. A packet naming any other command replaces the command after an existing wrapper, keeping the wrapper in place.
- OMP now accepts `ESC _ pi:keyword;<word> ESC \` input packets to place a standalone word such as `ultrathink`, `orchestrate`, or `workflowz` at the start of the composer message (after a leading slash command when present).
...
- Added `tui.pinComposerBottom` setting (default: on) to keep the input editor and status line docked to the bottom of the screen, with blank space between the transcript and the composer instead of the prompt floating up when tool output collapses.
- Prompt templates invoked from `.omp/prompts/` (e.g. `/name`) now collapse to a one-line card in the TUI instead of dumping the full expanded text; ctrl+o expands it.
