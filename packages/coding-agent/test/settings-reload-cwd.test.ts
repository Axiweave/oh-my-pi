...
it("layers a project modelProfile selector over the global one", async () => {
	...
	try {
		...
		expect(cfgModelProfile.get(withProject)).toBe("projectone");
		...
		expect(cfgModelProfile.get(withoutProject)).toBe("globalone");
	} finally {
		AgentStorage.close();
		if (fs.existsSync(testDir)) removeSyncWithRetries(testDir);
	}
...
describe("Settings.reloadForCwd", () => {
	...
	describe("project layer (on disk)", () => {
		...
		it("writes the project modelProfile selector alongside existing project roles", async () => {
			...
			expect(cfgModelProfile.get(settings)).toBe("work");
			await settings.flush();
