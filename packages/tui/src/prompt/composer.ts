...
import { sliceWithWidth, truncateToWidth, visibleWidth, wrapTextWithAnsi } from "../utils";
...
export class Composer implements TerminalFrameProvider {
	...
	renderFrame(viewport: ViewportSize): TerminalFramePlan {
		...
		if (transcriptIndex < 0) {
			...
			if (this.#preferences.pinBottom && spare > 0) {
				// Keep the prepaint header at the same row used after runtime adoption.
				// Only the input tail moves to the bottom until a transcript can absorb the gap.
				const headerRows = this.#runtimeMounted ? 0 : this.#header.render(width).length;
				return {
					viewport: [
						...rendered.slice(0, headerRows),
						...new Array(spare).fill(""),
						...rendered.slice(headerRows),
					],
				};
			}
	...
	acknowledgeHistory(id: number): void {
		...
		if (offered.kind === "replay" || this.#preferences.streamingScrollback) this.ui.requestRender();
	}
