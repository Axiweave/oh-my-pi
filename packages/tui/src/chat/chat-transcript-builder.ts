...
/**
 * Literal user text attached to a visible synthetic developer prompt (mirrors
 * `syntheticPromptDisplayText` in `session/messages.ts`, duplicated here since
 * this package cannot depend on the coding-agent session module).
 */
function developerPromptDisplayText(message: AgentMessage): string | undefined {
	if (message.role !== "developer") return undefined;
	const displayText = (message as AgentMessage & { displayText?: unknown }).displayText;
	...
	#appendChatMessage(message: AgentMessage): void {
		...
		switch (message.role) {
			...
			case "developer": {
				...
				const developerDisplayText = message.role === "developer" ? developerPromptDisplayText(message) : undefined;
				const userText = message.role === "user" ? textContent(message.content) : (developerDisplayText ?? "");
				if (userText) {
					const isSynthetic =
					...
					const collapseCommandCards = displayPreferences.collapseCommandCards;
					const templateName =
