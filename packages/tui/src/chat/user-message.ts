import { applyBackgroundToLine, padding, sliceByColumn, visibleWidth } from "../utils";
...
export class CollapsedSyntheticMessageComponent implements Component {
	...
	constructor(text: string, imageLinks?: readonly (string | undefined)[], label?: string, commandCard = false) {
		this.#text = text;
		...
		this.#disclosure = new Disclosure({
			summary: new SyntheticSummary(summarizeSyntheticInput(text, label)),
	...
	setIgnoreTight(ignore: boolean): this {
		...
		this.#prompt?.setIgnoreTight?.(ignore);
		return this;
	}
	...
	render(width: number): readonly string[] {
		const promptLines = this.#prompt ? this.#prompt.render(width) : [];
		return [...promptLines, ...this.#disclosure.render(width)];
	}
}
