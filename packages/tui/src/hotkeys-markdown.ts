...
export function buildHotkeysMarkdown(bindings: HotkeysMarkdownBindings): string {
	...
	return [
		...
		`| \`${hotkeyLabel(bindings, "app.model.cycleProfileForward")}\` | Choose model profile (cycle forward in cycling style) |`,
		`| \`${hotkeyLabel(bindings, "app.model.cycleProfileBackward")}\` | Choose model profile (cycle backward in cycling style) |`,
		`| \`${hotkeyLabel(bindings, "app.model.toggleCyber")}\` | Toggle cyber mode |`,
