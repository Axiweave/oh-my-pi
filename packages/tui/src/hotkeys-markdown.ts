...
export function buildHotkeysMarkdown(bindings: HotkeysMarkdownBindings): string {
	...
	return [
		...
		`| \`${hotkeyLabel(bindings, "app.model.cycleProfileForward")}\` | Cycle model profiles (whole role bundle) |`,
		`| \`${hotkeyLabel(bindings, "app.model.cycleProfileBackward")}\` | Cycle model profiles (backward) |`,
