...
export class ImageBudget {
	...
	#supersedeId(id: number): void {
		// A transmit queued by a discarded pass never reached the terminal, so
		// cancel it instead of transmitting then purging — the same split as
		// {@link #retire}, scoped to the surface this pass composes.
		if (!this.#pendingTransmits[this.#surface].delete(id)) this.#purgeIds.push(id);
		this.#transmitted[this.#surface].delete(id);
		// The other surface may still show this id, so its placement ledger and
		// key survive; {@link #forgetKeyForId} refuses while it is resident.
