...
import { usesClaudeFooter } from "../components/composer/preferences";
...
export class StatusLineComponent<TSession extends StatusLineSession = StatusLineSession> implements Component {
	...
	setPlanModeStatus(status: NonNullable<SegmentContext["planMode"]> | undefined): void {
		const next = status ?? null;
		if (
			this.#planModeStatus === next ||
			(this.#planModeStatus?.enabled === next?.enabled &&
				this.#planModeStatus?.paused === next?.paused &&
				this.#planModeStatus?.workflow === next?.workflow)
		) {
			return;
		}
		this.#planModeStatus = next;
	...
	#buildSegmentContext(
		...
		return {
			...
			ideSelection: this.host.ideSelection(),
			ideFile: this.host.ideFile(),
	...
	setComposerStyle(
		...
		this.#claudeFooterMode = usesClaudeFooter(style);
		this.#syncPricingTimer();
	}
	...
	getPreviewLines(
		...
		const claudeFooter = style === undefined ? this.#claudeFooterMode : usesClaudeFooter(style);
	...
	render(width: number): readonly string[] {
		...
		if (!this.#claudeFooterMode && showHooks && this.#sortedHookStatuses.length > 0) {
	...
	renderClaudeFooter(width: number, _previewTitle?: string): string[] {
		const settings = this.#resolveSettings();
		const ctx = this.#buildSegmentContext(width, settings.segmentOptions, true, true, false, Date.now());
		...
		if (hints) lines.push(theme.fg("muted", hints));
