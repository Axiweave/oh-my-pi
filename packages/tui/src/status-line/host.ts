...
/** Latest IDE text selection (Emacs/VS Code), mirroring `IDESelection` from the coding-agent's `ide` MCP integration. */
...
export interface StatusLineSession {
	...
	/** Active `modelProfiles` bundle name, or undefined when no profile is selected. */
	activeModelProfile?: string;
	/** Settings surface the `model_profile` segment reads to resolve the active bundle's roles. */
...
export interface StatusLineHost<TSession extends StatusLineSession = StatusLineSession> {
	...
	/** Latest IDE text selection, or null when none is active. */
