...
- In `picker` style, either profile-switch key opens the same searchable list of configured profile names and highlights the active profile.
- Filter text narrows the list. An unmatched filter cannot select a profile. Escape cancels without changing session state.
